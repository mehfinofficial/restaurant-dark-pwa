...
// ===============================
// PWA INSTALL POPUP SYSTEM
// ===============================
let deferredPrompt = null;

// Catch browser prompt event
window.addEventListener("beforeinstallprompt", (e) => {
    e.preventDefault();
    deferredPrompt = e;
});

// Helper function: days difference
function daysPassed(timestamp) {
    return (Date.now() - timestamp) / (1000 * 60 * 60 * 24);
}

// Main logic runner
function checkInstallPopup() {

    // If PWA already installed → never show
    if (window.matchMedia("(display-mode: standalone)").matches ||
        localStorage.getItem("pwaInstalled") === "true") return;

    const dismissedAt = localStorage.getItem("installDismissedAt");

    // If dismissed before → wait 3 days
    if (dismissedAt && daysPassed(parseInt(dismissedAt)) < 3) {
        return;
    }

    // FIRST VISIT DELAY: 15 seconds
    setTimeout(() => {
        showInstallPopup();
    }, 15000);
}
...
// Button actions
document.getElementById("installPopupInstallBtn").addEventListener("click", async () => {
...
document.getElementById("installPopupLaterBtn").addEventListener("click", () => {
...
// RUN ON PAGE LOAD
window.addEventListener("load", checkInstallPopup);
